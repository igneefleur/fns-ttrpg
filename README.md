...
Cette branche vient d'être ouverte depuis `jjk` : l'outillage est adapté à MIA
(fiche, extension, plateau, chaîne de publication), **les règles ne le sont pas
encore**. `docs/content/regles/index.md` porte toujours le corps des règles JJK,
gardé comme échafaudage parce que `hooks/mia_creation.py` en tire au build les
données du créateur et de la fiche. Ces règles-là sont l'œuvre de Qyu : elles
doivent être remplacées par celles d'Erua avant toute publication.

Deux autres choses attendent leur tour :

- **la signature Mozilla**, coupée tant que l'add-on `mia-roll20@igneefleur`
  n'existe pas chez eux. Sur CETTE branche elle est automatique à chaque envoi :
  la garde est donc essentielle, et la marche à suivre pour la lever est en tête
...
Tant que ces règles sont celles de JJK, le travail se fait sur `mia-beta`, pas
ici : cette branche-ci est celle des joueurs.
