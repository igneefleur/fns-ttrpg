/* Moteur de migration de la fiche JJK : montée ET descente de schéma.
 *
 ...
 * le navigateur (window.JjkMigr) que dans node (module.exports), pour que
...
(function (global) {
  ...
  // (docs/javascripts/jjk-fiche.js) repasse sur l'état à chaque chargement,
  ...
  //
  // Relevé fait sur jjk-fiche.js le 2026-08-03 (numéros de ligne indicatifs,
  ...
  // JjkMigr.IRREVERSIBLE("la fusion des groupes ne se défait pas") ».
  function Irreversible(raison) {
    var e = new Error(String(raison || "descente impossible"));
    e.name = "JjkIrreversible";
    e.irreversible = true;
  ...
  function Registre() {
    ...
    function ajouter(def) {
      ...
      if (typeof def.descendre !== "function") {
        throw new Error("migration " + s + " : descendre() manque (un pas irréversible lève JjkMigr.IRREVERSIBLE)");
      }
  ...
  var JjkMigr = Registre();

  /* ------------------------------------------------------------------
   * LA CHAÎNE PUBLIÉE
   *
   * Socle : schéma 1. Toutes les fiches en circulation le portent (state.v
   * vaut 1 depuis blank()), aucune n'a jamais porté autre chose.
   *
   * Les migrations d'AVANT 3.0.0 (renommages de compétences, inventaire en
   * texte libre devenu objets illustrés, divers de caracs et de compétences
   * repliés en modificateur unique, passifs devenus techniques) restent EN
   * LIGNE dans normalize() du bundle. Elles n'ont pas leur place ici :
   *   - elles sont idempotentes et éprouvées sur le parc réel ;
   *   - elles n'ont pas de cible de DESCENTE, et n'en auront jamais : elles
   *     réparent des états sans numéro de schéma, qu'aucun « de » ne saurait
   *     désigner ;
   *   - les déplacer ici les ferait sauter sur toute fiche déjà marquée v1,
   *     c'est-à-dire sur tout le parc.
   * Ce moteur ne prend donc en charge que ce qui vient APRÈS 3.0.0.
   * ------------------------------------------------------------------ */

  JjkMigr.ajouter({
    schema: 2,
    titre: "Réservé (3.0.0)",
    notes: "Aucun changement de structure : la fiche se contente de changer de numéro.",
    // 3.0.0 sort avec le parc entier en schéma 1, et le manifeste annonce
    // déjà schéma 3. La chaîne doit être CONTIGUË, sinon appliquer(1, 3)
    // refuse de partir : d'où ces deux pas vides, qui ne touchent à rien.
    monter: function () {},
    descendre: function () {}
  });
  ...
  JjkMigr.ajouter({
    schema: 3,
    titre: "Fiche 3.0.0",
    notes: "Aucun changement de structure : versionnage, grenier et journal de migration s'ajoutent autour de l'état, pas dedans.",
    // Même raison qu'au pas 2. Les clés grenier et vHist n'ont pas besoin
    // d'être créées ici : elles vivent à la racine, que normalize() conserve,
    // et n'apparaissent que le jour où un pas s'en sert.
    monter: function () {},
    descendre: function () {}
  });

  global.JjkMigr = JjkMigr;
  if (typeof module === "object" && module && module.exports) module.exports = JjkMigr;
