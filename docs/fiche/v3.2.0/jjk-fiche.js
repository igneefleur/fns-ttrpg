...
(function () {
  ...
  function deplaceModule(id, onglet, colonne, avantId) {
    ...
    // Revenir à sa place native EFFACE l'entrée plutôt que d'y ranger la place
    // d'origine : la disposition reste éparse, et un module que la fiche
    ...
    // la table en mémoire suit TOUT DE SUITE : memeColonne() lit modules[],
    // et l'ordre qu'on s'apprête à écrire s'appuie dessus
    var i = rangModule(id);
    if (i >= 0) {
      var copie = {};
      Object.keys(modules[i]).forEach(function (k) { copie[k] = modules[i][k]; });
      copie.onglet = onglet;
  ...
  function buildModules() {
    ...
    TABS.forEach(function (t) {
      ...
      var cols = colonnesDe(t.id) || {};
      var noms = Object.keys(cols);
      if (!noms.length) noms = ["gauche"];
      ...
      noms.forEach(function (c, i) {
        var z = zone(t.id, c, LIB_COLONNES[c] || capFirst(c));
        dedans.forEach(function (m) {
          ...
          var perdue = colonneInconnue(m);
          var ici = perdue ? (i === 0) : (m.colonne === c);
          if (!ici) return;
          vus[m.id] = 1;
          z.liste.appendChild(carte(m, perdue
            ? "colonne « " + m.colonne + " » inconnue dans cet onglet : ce module ne s'affiche nulle part"
            : ""));
        });
        rangee.appendChild(z.bloc);
      });
      plan.appendChild(rangee);
    });
    ...
    if (perdus.length) {
      ...
      perdus.forEach(function (m) {
        zp.liste.appendChild(carte(m, "onglet « " + m.onglet + " » inconnu : ce module ne s'affiche nulle part"));
      });
