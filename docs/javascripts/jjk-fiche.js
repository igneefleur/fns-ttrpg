...
(function () {
  ...
  function blank() {
    return {
      ...
      inv: {
        ...
        comptes: [true],
        // cols et nom appartiennent à l'inventaire à TUILES, remplacé par un
        // registre en lignes : ils restent inertes pour les archives, ne pas
        // les réemployer. vign commande la colonne des vignettes.
  ...
  // ---------- inventaire : un registre d'objets, par contenants ----------
  // Les objets étaient des tuiles carrées dont le nom vivait à 0.5rem, la plus
  // petite police de la feuille ; ce sont maintenant des LIGNES au corps du
  // document, à pistes fixes, comme les rangées de compétences. Un groupe est
  // un CONTENANT coiffé d'un bandeau qui porte son nom, son nombre d'objets,
  // son poids et sa case « Compté » : décochée, le contenant est posé au sol,
  // son poids sort du poids porté, mais ses objets restent entiers, lisibles,
  // déplaçables et donnables. Le détail d'un objet ne vit plus dans une colonne
  // de 34 % où les champs se marchaient dessus : il se déplie sous sa propre
  // ligne, sur toute la largeur du module.
  ...
  function invObjets(container, renderRef) {
    ...
    // sel est une RÉFÉRENCE d'objet, plus un index : un objet donné au tchat,
    // retiré ou déplacé peut quitter la liste entre deux rendus, et un index
    // devait alors se recaler à la main en quatre endroits.
    var sel = null;
    var dragIt = null;       // l'objet en cours de glissement
    var editGi = null;       // groupe à ouvrir en édition de nom au prochain render
    // Registres locaux, sur le modèle de l'ancien tileRefs : le module ne
    // pousse RIEN dans hooks, il tient ses lignes à jour lui-même pour ne pas
    // se redessiner (et perdre le focus) à chaque frappe.
    var rowRefs = [];        // { it: objet, maj: fonction }
    var grpRefs = [];        // { gi: index, maj: fonction }
    var tiroirRef = { it: null, maj: null };   // le tiroir ouvert, s'il y en a un

    // réglages d'affichage du module, en mode édition seulement
    var optRow = el("div", "pc-inv-opts pc-edit-only");
    optRow.appendChild(el("span", "lbl", "Afficher"));
    [["vign", "Vignettes"], ["qte", "Quantité"], ["poids", "Poids"], ["total", "Total"]].forEach(function (o) {
      var chip = el("span", "pc-chip");
      chip.textContent = o[1];
      chip.title = o[0] === "total"
        ? "Afficher le total de chaque pile, et le poids en bas du module"
        : "Afficher la colonne « " + o[1] + " »";
    ...
    var conts = el("div", "pc-inv-conts");
    var pied = el("div", "pc-inv-total");

    // Le pied ne dit QUE les objets : les armes et les armures pèsent aussi
    // dans poidsPorte(), et un pied qui annoncerait « poids porté » tout court
    // contredirait la carte du personnage. Le poids de JJK est un nombre SANS
    // unité : jamais de « kg ».
    function majPied() {
      var porte = poidsObjets(true), pose = poidsObjets(false);
      pied.textContent = "Poids des objets portés : " + fmtP(porte) +
        (pose ? " · posés : " + fmtP(pose) : "");
      pied.style.display = O.total && (porte || pose) ? "" : "none";
    }
    function majLigne(it) {
      rowRefs.forEach(function (r) { if (r.it === it) r.maj(); });
    }
    function majGroupe(gi) {
      grpRefs.forEach(function (r) { if (r.gi === gi) r.maj(); });
    }
    // le tiroir n'est mis à jour que s'il montre CET objet : sinon ses champs
    // afficheraient la quantité d'un voisin
    function majTiroir(it) {
      if (tiroirRef.it === it && tiroirRef.maj) tiroirRef.maj();
    }
    // toute écriture sur un objet touche sa ligne, son bandeau, le pied du
    // module et le poids porté : une seule chaîne, jamais un render()
    function majObjet(it) {
      majLigne(it);
      majGroupe(it.groupe);
      majTiroir(it);
      majPied();
    ...
    function moveTo(it, gi, cible) {
      // déplace l'objet dans le groupe gi, juste avant `cible` (null : à la fin).
      // La position cible se recalcule APRÈS le retrait : retirer l'objet déplacé
      // décale les index de tout ce qui le suivait.
      var from = items.indexOf(it);
      if (from < 0) return;
    ...
    // ---- une ligne d'objet ----
    function ligne(it, impair) {
      var row = el("div", "pc-inv-row obj" + (impair ? " odd" : "") + (sel === it ? " sel" : ""));

      // vignette : repère, pas sujet. Sa piste peut tomber à 0 sans qu'elle
      // quitte le DOM, donc sans décaler les colonnes suivantes.
      var vig = el("span", "vig", it.img ? null : "?");
      if (it.img) {
        var im = el("img");
        im.alt = ""; im.draggable = false; im.src = it.img;
        vig.appendChild(im);
      }
      row.appendChild(vig);
      ...
      var lbl = el("span", "lbl");
      nmc.appendChild(lbl);
      // la croix vit DANS la cellule du nom, pas dans une piste à elle : une
      // piste dédiée laisserait un trou dans la grille hors édition
      var del = el("button", "x pc-edit-only", "✕");
      del.type = "button";
      ...
      del.addEventListener("click", function (e) {
        ...
        function faire() {
          ...
          if (here >= 0) items.splice(here, 1);
          render();
          refresh();
        }
        if (it.nom || it.desc)
          confirmer("Retirer un objet", "« " + (it.nom || "cet objet") + " » quitte l'inventaire.", "Retirer", faire);
        else faire();
      });
      nmc.appendChild(del);
      row.appendChild(nmc);

      // Quantité : GESTE DE JEU, donc pas de .pc-edit-field. Décimale (2.5 m de
      // corde, une demi-ration).
      var qIn = el("input", "qte");
      ...
      qIn.addEventListener("input", function () {
        it.qte = pnum(qIn.value);
        majObjet(it);   // jamais render() : la ligne recréée perdrait le focus
      });
      row.appendChild(qIn);

      // Poids unitaire : CONSTRUCTION, il décrit l'objet et non la scène.
      var pIn = el("input", "pds pc-edit-field");
      pIn.type = "text"; pIn.inputMode = "decimal"; pIn.placeholder = "0";
      pIn.title = "Poids unitaire";
      pIn.addEventListener("click", function (e) { e.stopPropagation(); });
      pIn.addEventListener("input", function () {
        it.poids = pnum(pIn.value);
        majObjet(it);
      });
      pIn.addEventListener("blur", function () { pIn.value = it.poids ? fmtP(it.poids) : ""; });
      row.appendChild(pIn);
      ...
      function maj() {
        var nom = it.nom || "Objet";
        lbl.textContent = nom;
        lbl.classList.toggle("vide", !it.nom);
        row.title = nom;   // le nom est ellipsé : le complet vit dans le title
        // garde-fou sur le champ en cours de frappe : le reformater sous les
        // doigts déplacerait le curseur à chaque caractère
        if (document.activeElement !== qIn) qIn.value = fmtP(it.qte);
        if (document.activeElement !== pIn) pIn.value = it.poids ? fmtP(it.poids) : "";
        var t = pnum(it.qte) * pnum(it.poids);
        tot.textContent = t ? fmtP(t) : "";
      }
      maj();
      rowRefs.push({ it: it, maj: maj });

      // un seul tiroir ouvert à la fois ; le second clic sur la MÊME ligne le
      // referme, c'est le geste unique (pas de croix de fermeture en plus)
      row.addEventListener("click", function () {
        sel = sel === it ? null : it;
        render();
      });

      // draggable n'est PAS posé en dur : sans cette précaution Firefox arbitre
      // mal entre la sélection de texte d'un champ et le glisser du parent
      row.addEventListener("mousedown", function (e) {
        var t = e.target.tagName;
        row.draggable = isEdit("inv") && t !== "INPUT" && t !== "BUTTON" && t !== "SELECT" && t !== "LABEL";
      });
      ...
      row.addEventListener("dragend", function () { row.draggable = false; dragIt = null; render(); });
      row.addEventListener("dragover", function (e) {
        if (!dragIt) return;
        // lâcher sur soi-même : cible invalide, et on N'EN LAISSE PAS la liste
        // du groupe la valider (sinon l'objet saute en fin de groupe)
        if (dragIt === it) { e.stopPropagation(); return; }
        e.preventDefault();
        e.stopPropagation();
        var r = row.getBoundingClientRect();
        var haut = e.clientY < r.top + r.height / 2;
        row.classList.toggle("over-t", haut);
        row.classList.toggle("over-b", !haut);
      });
      row.addEventListener("dragleave", function () { row.classList.remove("over-t", "over-b"); });
      row.addEventListener("drop", function (e) {
        ...
        e.stopPropagation();
        var r = row.getBoundingClientRect();
        var avant = e.clientY < r.top + r.height / 2;
        var deplace = dragIt; dragIt = null;
        // déposer SOUS une ligne = s'insérer avant la suivante du groupe.
        ...
        var cible = it;
        if (!avant) {
          var freres = items.filter(function (x) { return x.groupe === it.groupe && x !== deplace; });
          var k = freres.indexOf(it);
          cible = k >= 0 && k + 1 < freres.length ? freres[k + 1] : null;
        }
        moveTo(deplace, it.groupe, cible);
        render();
        refresh();
      });
      return row;
    ...
    // ---- le tiroir de détail, sous SA ligne, sur toute la largeur ----
    function tiroir(it) {
      var box = el("div", "pc-inv-detail");
      ...
      slider.max = fmtP(Math.max(10, it.qte));
      slider.value = it.qte;
      slider.step = "any";
      var qIn = el("input", "n");
      qIn.type = "number"; qIn.min = "0"; qIn.step = "any";
      ...
      // le curseur et le champ s'alimentent l'un l'autre : le double garde-fou
      // sur activeElement empêche d'écraser la frappe en cours
      function cadreQte() {
        if (+slider.max < it.qte) slider.max = fmtP(it.qte);
      ...
      // Les cinq petits champs tiennent dans UNE seule paire : la grille
      // auto-fit les répartit selon la place, sans requête de média, et un
      // champ masqué hors édition quitte la grille qui se recompose seule.
      ...
      gSel.addEventListener("change", function () {
        moveTo(it, clamp(num(gSel.value, 0), 0, G.length - 1), null);
        render();
        refresh();
      });
      ...
      pair.appendChild(fld("Identifiant", idIn, "pc-edit-only"));
      ...
      var urlFld = fld("Image (URL)", url, "w");
      var file = el("input");
      ...
      function majPile() {
        pile.textContent = "Total : " + fmtP(pnum(it.qte) * pnum(it.poids));
      ...
      actions.appendChild(chatBtn(
        ...
        function () {
          var q = bornerAct();
          return [
            // G[it.groupe] : le nom du groupe est une CHAÎNE, et le drapeau
            // « compté » vit à part précisément pour qu'il le reste ici
            ["Groupe", G[it.groupe]],
      ...
      tiroirRef.it = it;
      tiroirRef.maj = function () {
        cadreQte();
        majAct();
        majPile();
        if (document.activeElement !== pd) pd.value = it.poids ? fmtP(it.poids) : "";
      };
      return box;
    }
    ...
    function groupBox(gi) {
      var box = el("div", "pc-inv-grp" + (invCompte(gi) ? "" : " pose"));
      var head = el("div", "pc-inv-ghead");
      ...
      function editName() {
        ...
        // renommer ne touche PAS state.inv.comptes : le drapeau est rangé par
        // index, il survit donc au changement de nom
      ...
      if (G.length > 1) {
        ...
        delG.addEventListener("click", function () {
          var n = 0;
          items.forEach(function (o) { if (o.groupe === gi) n++; });
          function faire() {
            G.splice(gi, 1);
            // les DEUX tableaux se coupent ENSEMBLE, sinon le drapeau du groupe
            // suivant se décale d'un cran et un sac posé se remettrait à peser
          ...
          if (n) confirmer("Supprimer un groupe",
            "« " + G[gi] + " » contient " + fmtP(n) + (n > 1 ? " objets" : " objet") +
            ", qui rejoindront le premier groupe.", "Supprimer", faire);
          else faire();
        });
      ...
      var nb = el("span", "nb");
      head.appendChild(nb);
      // le poids du groupe ne dépend d'AUCUNE option d'affichage : il est
      // visible en permanence, c'est une demande et pas un réglage
      var pds = el("span", "pds");
      head.appendChild(pds);

      // GESTE DE JEU : poser son sac en pleine scène ne doit pas demander
      // d'ouvrir le rouage. La case ne porte donc ni .pc-edit-only (elle
      // disparaîtrait rouage fermé) ni .pc-edit-field (elle serait inerte), et
      // aucun garde isEdit() ne la protège. C'est le SEUL contrôle du module
      // dans ce cas : ne pas le « ranger » du côté construction plus tard.
      var lab = el("label", "pc-inv-cnt");
      lab.title = "Décoché : le groupe est posé, son poids ne compte plus dans le poids porté.";
      var cb = el("input");
      cb.type = "checkbox";
      cb.checked = invCompte(gi);
      cb.addEventListener("change", function () {
        state.inv.comptes[gi] = cb.checked;
        box.classList.toggle("pose", !cb.checked);
        majGroupe(gi);
        majPied();
        refresh();   // l'initiative, la carte et les mods suivent dans le geste
      });
      lab.addEventListener("click", function (e) { e.stopPropagation(); });
      lab.appendChild(cb);
      lab.appendChild(el("span", "t", "Compté"));
      ...
      function maj() {
        var n = 0;
        items.forEach(function (o) { if (o.groupe === gi) n++; });
        nb.textContent = n ? fmtP(n) + (n > 1 ? " objets" : " objet") : "";
        var p = poidsGroupe(gi);
        // posé : entre parenthèses (le CSS s'occupe de l'encre et de la graisse).
        // Groupe sans poids : chaîne vide, l'élément garde sa largeur et la case
        // ne bouge pas d'un groupe à l'autre.
        pds.textContent = p ? (invCompte(gi) ? fmtP(p) : "(" + fmtP(p) + ")") : "";
      }
      maj();
      grpRefs.push({ gi: gi, maj: maj });

      // la liste garde une hauteur même vide, sinon un contenant vidé ne
      // pourrait plus rien recevoir au glisser-déposer
      var liste = el("div", "pc-inv-liste");
      var n = 0;
      items.forEach(function (it) {
        if (it.groupe !== gi) return;
        if (!n) {
          var tete = el("div", "pc-inv-row head");
          tete.appendChild(el("span", "vig"));
          tete.appendChild(el("span", "nm", "Objet"));
          tete.appendChild(el("span", "qte", "Qté"));
          tete.appendChild(el("span", "pds", "Poids"));
          tete.appendChild(el("span", "tot", "Total"));
          liste.appendChild(tete);
        }
        liste.appendChild(ligne(it, n % 2 === 1));
        // le tiroir s'insère dans la liste, juste APRÈS sa ligne : il reste
        // dans son groupe et prend toute la largeur du module
        if (sel === it) liste.appendChild(tiroir(it));
        n++;
      });
      if (!n) liste.appendChild(el("div", "pc-inv-vide", "Aucun objet."));

      // la case d'ajout dit où l'objet va naître, ce qu'un bouton posé sous le
      // module ne dirait pas
      var add = el("button", "pc-inv-add pc-edit-only", "+ Objet");
      add.type = "button";
      add.title = "Ajouter un objet dans « " + G[gi] + " »";
      add.addEventListener("click", function () {
        var neuf = { nom: "", qte: 1, poids: 0, img: "", desc: "", id: "", achat: 0, vente: 0, groupe: gi };
        items.push(neuf);
        sel = neuf;
        render();
        refresh();
      });
      liste.appendChild(add);

      // déposer dans le vide de la liste ou sur le bandeau : fin de ce groupe
      function survol(e) {
        if (!dragIt) return;
        e.preventDefault();
      ...
      function poser(e) {
        ...
        e.preventDefault();
        var deplace = dragIt; dragIt = null;
        moveTo(deplace, gi, null);
        render();
        refresh();
      }
      liste.addEventListener("dragover", survol);
      liste.addEventListener("dragleave", function () { liste.classList.remove("survol"); });
      liste.addEventListener("drop", poser);
      head.addEventListener("dragover", survol);
      head.addEventListener("drop", poser);
      box.appendChild(liste);
      return box;
    }

    function render() {
      // sel est une référence, pas un index : un objet donné au tchat, retiré
      // ou déplacé peut avoir quitté la liste entre deux rendus
      if (items.indexOf(sel) < 0) sel = null;
      rowRefs = [];
      grpRefs = [];
      tiroirRef.it = null;
      tiroirRef.maj = null;
      // Les colonnes éteintes se pilotent par CLASSE et non par un style
      // inline : un style inline gagnerait sur la requête de média des 44em,
      // qui doit pouvoir éteindre le poids unitaire quelle que soit l'option.
      conts.className = "pc-inv-conts" +
        (O.vign ? "" : " sans-vig") + (O.qte ? "" : " sans-qte") +
        (O.poids ? "" : " sans-pds") + (O.total ? "" : " sans-tot");
      conts.innerHTML = "";
      G.forEach(function (_, gi) { conts.appendChild(groupBox(gi)); });
      var addG = miniBtn("+ Groupe", "Ajouter un groupe d'objets", function () {
        G.push("Groupe");
        // les deux tableaux se poussent ENSEMBLE : un drapeau manquant se
        // rattraperait au prochain chargement, mais pas avant
        state.inv.comptes.push(true);
        editGi = G.length - 1;   // le nouveau groupe s'ouvre en édition de nom
        render();
        refresh();
      }, "pc-edit-only");
      addG.classList.add("pc-inv-addg");
      conts.appendChild(addG);
      majPied();
      // Les éléments recréés par un rendu interne naissent hors du cycle du
      // rouage : sans ce rappel, un .pc-edit-only neuf reste visible en jeu et
      // un .pc-edit-field neuf reste actif.
      applyEdit(container, "inv");
    }
    if (renderRef) renderRef.fn = render;
    invRender = render;   // un objet reçu du tchat redessine l'inventaire
    render();
    container.appendChild(conts);
