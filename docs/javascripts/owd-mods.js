/* Moteur de mods de la fiche Outward : empreinte, consentement, exécution.
 *
 ...
 *   Ce qui BORNE un mod est ailleurs : c'est le pont de l'extension Roll20
 *   (seuls les attributs préfixés owd_ sont écrits, seulement sur le
 ...
 * rend des bilans. Toute l'interface est dans owd-fiche.js. Il est facultatif
 * de naissance : si window.OwdMods manque, la fiche s'ouvre sans mods,
 * exactement comme elle s'ouvre sans window.OwdMigr.
 */
(function (global) {
  "use strict";

  // Le VRAI localStorage du navigateur, et lui seul (voir magasin()).
  var CLE_AVIS = "owd.mods.avis";
  ...
  // window.__owdLocalStorage : ce shim écrit dans les Attributes du
  ...
  // (OwdMods.lireVersion, OwdMods.compareVersions). La fiche, les mods et les
  // sondes s'en servent au lieu de recopier chacun leur expression régulière.
  // La fiche JJK, dont ce moteur est le portage, tenait la sienne, et elle est
  // restée en arrière : le jour du suffixe de beta, son formulaire s'est mis à
  // refuser le numéro qu'il proposait lui-même en filigrane. Cette fiche-ci
  // naît sur la beta, donc AVEC le suffixe : la faute serait immédiate.
  //
  // L'amorceur (owd-roll20-boot.js) ne PEUT pas s'en servir : il tranche quelle
  ...
  // « 1.2.9b » rend { x: 1, y: 2, z: 9, beta: true }, et null sur ce qui n'est
  // pas un numéro.
  //
  // Le suffixe « b » de la branche beta est COLLÉ au dernier nombre, sans
  // séparateur : ce n'est pas une pré-version au sens semver, et un motif qui
  // n'accepterait un suffixe que derrière un « - » ou un « + » rendrait null
  // sur « 1.0.0b », c'est-à-dire sur le numéro que ce dépôt publie
  // aujourd'hui. Silencieusement, ce qui est le pire : compareVersions rend 0
  // sur un illisible, donc le verrou « pour la fiche » de TOUS les mods
  // s'éteindrait sans un message. Un suffixe semver (« 1.1.0-beta ») reste
  // toléré et reste lu comme sa version de base ; « beta » ne dit QUE le b du
  // contrat, parce que c'est lui seul qui désigne le site beta.
  ...
  // Comparer des CHAÎNES rendrait "1.10.0" inférieur à "1.9.0", et un mod
  // parfaitement compatible se déclarerait « trop récent » à l'infini. D'où ce
  // comparateur à trois nombres.
  //
  // Le suffixe de beta ne pèse RIEN dans le rang : « 1.2.9b » et « 1.2.9 »
  ...
  // que le site stable du même numéro, et un mod « pour: 1.2.9 » refuserait de
  // tourner sur la beta qui le publie.
  ...
  //
  // UN SEUL ID, TROIS CHEMINS : cette fonction, le normalize() de owd-fiche.js
  // et l'idMod() de son formulaire doivent donner le MÊME id. Sans quoi
  // l'empreinte changerait selon le chemin pris, et le joueur réautoriserait
  // un mod qu'il connaît déjà.
  function idPropre(v) {
  ...
  function normalise(mods) {
    ...
    for (i = 0; i < mods.length; i++) {
      ...
      try {
        ...
        // non. Un « 1.x » écrit à la main n'est pas une version au sens de
  ...
  // manque (appel sans infos, objet Owd incomplet), le verrou correspondant
  ...
  // qui arriverait si lireVersion butait sur le suffixe.
  //
  ...
  function repere(Owd, infos) {
    ...
    try {
      ...
      if (v === null && Owd && lireVersion(Owd.version)) v = chaine(Owd.version).trim();
      if (s === null && Owd) {
        n = entierPositif(Owd.schema);
        if (n !== null) s = n;
      }
    } catch (e) {}
    return { version: v, schema: s, crochet: crochetDe(Owd, infos) };
  }

  // Le contrat veut que les filtres posés PENDANT l'exécution d'un mod lui
  // soient attribués (c'est ce qui range le message d'un filtre retiré dans
  // Owd.etat(id).erreur). Or « proprietaireCourant » est une variable interne
  // de owd-fiche.js : ce moteur ne peut que la lui demander. Si la fiche pose
  // un crochet, il sert ; sinon tout marche pareil et le propriétaire reste
  // « ? ». Le crochet reçoit l'id du mod, puis null quand il rend la main.
  function crochetDe(Owd, infos) {
    try {
      if (infos && typeof infos.proprietaire === "function") return infos.proprietaire;
      if (Owd && typeof Owd.__proprietaire === "function") return Owd.__proprietaire;
    } catch (e) {}
    return null;
  }
  ...
  // lui, montre le numéro BRUT (« celle-ci est en 1.0.0b ») alors que la
  ...
  // variables. Un mod ne voit donc rien d'ici ; il ne dispose que de Owd, de
  // ctx, et de ce que la page expose de toute façon.
  function lance(mod, Owd, rep) {
    var fn, ctx, rendre;
    try {
      fn = new Function("Owd", "ctx", mod.src);
    } catch (e) {
  ...
  function execute(mods, Owd, infos) {
    var liste = normalise(mods), rep = repere(Owd, infos), bilan = [], i, m, emp, v, res;
  ...
  //
  // LE PASSER N'EST PAS UNE COQUETTERIE : sans lui, le moteur saute ses
  // contrôles, un mod « pour: 2.0.0 » est annoncé « pas autorisé », le joueur
  // l'autorise, et le bloc Mods lui répond « trop récent ». Le oui ainsi
  // arraché dort dans le navigateur et s'appliquerait tout seul le jour de la
  // 2.0.0, sans que personne ne l'ait voulu.
  ...
  var OwdMods = {
    ...
    // motifs pour une seule décision, c'est ainsi qu'un formulaire de mods
    // finit par refuser le numéro qu'il propose lui-même en filigrane.
    lireVersion: lireVersion,
    compareVersions: compareVersions
  };

  global.OwdMods = OwdMods;
  // Exporté aussi pour node, comme OwdMigr : le moteur n'a besoin d'aucun DOM,
  // un script de test peut donc l'éprouver hors du navigateur.
  if (typeof module === "object" && module && module.exports) module.exports = OwdMods;
