/* Moteur de mods de la fiche JJK : empreinte, consentement, exécution.
 *
 ...
 *   (seuls les attributs préfixés jjk_ sont écrits, seulement sur le
 ...
 * rend des bilans. Toute l'interface est dans jjk-fiche.js. Il est facultatif
 * de naissance : si window.JjkMods manque, la fiche s'ouvre sans mods,
 * exactement comme elle s'ouvre sans window.JjkMigr.
 */
(function (global) {
  "use strict";

  // Le VRAI localStorage du navigateur, et lui seul (voir magasin()).
  var CLE_AVIS = "jjk.mods.avis";
  ...
  // window.__jjkLocalStorage : ce shim écrit dans les Attributes du
  ...
  function repere(Jjk, infos) {
    ...
    return { version: v, schema: s, crochet: crochetDe(Jjk, infos) };
  }
  ...
  // dans Jjk.etat(id).erreur). Or « proprietaireCourant » est une variable
  // interne de jjk-fiche.js : ce moteur ne peut que la lui demander. Si la
  ...
  // il rend la main.
  function crochetDe(Jjk, infos) {
    try {
      if (infos && typeof infos.proprietaire === "function") return infos.proprietaire;
      if (Jjk && typeof Jjk.__proprietaire === "function") return Jjk.__proprietaire;
    } catch (e) {}
    return null;
  }
  ...
  // variables. Un mod ne voit donc rien d'ici ; il ne dispose que de Jjk, de
  // ctx, et de ce que la page expose de toute façon.
  function lance(mod, Jjk, rep) {
    var fn, ctx, rendre;
    try {
      fn = new Function("Jjk", "ctx", mod.src);
    } catch (e) {
  ...
  function execute(mods, Jjk, infos) {
    var liste = normalise(mods), rep = repere(Jjk, infos), bilan = [], i, m, emp, v, res;
  ...
  // Exporté aussi pour node, comme JjkMigr : le moteur n'a besoin d'aucun DOM,
  // un script de test peut donc l'éprouver hors du navigateur.
  if (typeof module === "object" && module && module.exports) module.exports = JjkMods;
