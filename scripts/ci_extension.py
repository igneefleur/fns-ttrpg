"""Re-signature de l'extension Outward en CI, sur demande expresse.

Appelé par le workflow de la branche, et SEULEMENT quand la case « signer »
d'une relance manuelle est cochée : un push ordinaire ne signe jamais rien. Le
quota de soumissions Mozilla se compte à la dizaine par jour et il est PARTAGÉ
avec l'extension de la branche sœur ; une signature automatique à chaque push
le viderait pour rien, puisque l'extension est une COQUILLE et que la fiche,
elle, se déploie sans elle.

Logique :

  1. Contrôles du paquet (build_extension.verifie()), avant tout le reste.
...
deux moitiés n'ont pas changé depuis la 1.0.0 ».

    python scripts/ci_extension.py --etat   # dit ce qui a changé, n'écrit rien

LE NUMÉRO SE POSE, IL NE S'INCRÉMENTE PAS À LA MAIN. Le projet n'a qu'une ligne
de versions : la fiche du site stable, celle de la beta et l'extension visent le
même numéro. La coquille prend donc celui que publie docs/owd-manifeste.json,
...
validation, quota consommé. On passe alors au QUATRIÈME NOMBRE (le compteur de
signatures), ou, à défaut, on monte d'un cran Z au-dessus du plancher en le
DISANT en clair dans le journal du run. Et si le plancher lui-même est
...
PREMIER ENVOI : les manifests partent en « 1.0.0.0 ». Ce numéro-là ne sera
JAMAIS soumis tel quel — Firefox et Chrome complètent de zéros et tiennent
« 1.0.0.0 » pour égal à « 1.0.0 » — et c'est voulu : le plancher vaut alors
1.0.0.0, la cible tirée du manifeste vaut 1.0.0 (pas au-dessus), donc
signature_suivante() rend « 1.0.0.1 », et c'est ce numéro qui part chez Mozilla
et qui est réécrit dans les deux manifests. Le compteur de signatures naît
ainsi de lui-même, sans que personne ait rien à poser.
...
# numéro : recodé ici, deux chemins pour un même fichier finiraient par ne plus
# désigner le même.
...
def content_hash():
    ...
    2. LES FINS DE LIGNE. Sans .gitattributes, le dépôt se décoche en CRLF
       sous Windows et en LF sous Linux : lire le disque donne deux suites
       d'octets pour un fichier identique. Les fichiers texte sont donc
       ramenés en LF avant d'être hachés. Les binaires (icônes, polices)
       passent intacts, reconnus à l'octet nul que git utilise lui-même comme
       indice : les normaliser les abîmerait.

    Une signature faite depuis le poste enregistrerait sinon une empreinte que
    la CI ne retrouverait jamais : au déploiement suivant elle croirait
    l'extension modifiée et re-signerait, mordant sur le quota AMO pour rien.

    3. LES NUMÉROS, TOUS LES NUMÉROS. Le champ "version" des manifests est
       neutralisé ; parties.js, qui déclare le numéro de chaque partie, l'est
       pour LA MÊME RAISON et il faut y insister, parce que le piège y est
       pire. Ce fichier vit DANS l'extension et c'est CE script qui l'écrit, à
       chaque signature : compté dans l'empreinte, il la ferait changer à tout
       coup, le déploiement suivant croirait l'extension modifiée,
       re-signerait, réécrirait le fichier, changerait l'empreinte… une boucle
       qui tourne toute seule, un quota AMO par tour, sur un quota qui se
       compte à la dizaine par jour. Son NOM reste haché : le fichier doit être
       là, et sa disparition, elle, est bien un changement de paquet.
    """
    h = hashlib.sha256()
...
def empreinte_partie(nom):
    ...
    jamais avoir la même empreinte, alors même que leurs quelques lignes de
...
def bump_version(old):
    """Le cran Z au-dessus, RETENUE comprise. Ce n'est PAS le chemin normal.

    C'est le repli du cas de recul : quand la version du projet n'est pas
    au-dessus de ce qui est déjà signé et que le compteur de signatures ne
    s'applique pas, il faut bien proposer quelque chose de neuf à Mozilla. Sans
    la retenue, un 1.6.999 produirait 1.6.1000, hors du domaine 0 à 999 que le
    contrat borne.

    UN PLANCHER ILLISIBLE ARRÊTE TOUT. Repartir de « 0.0.1 » sur un simple
    avertissement proposerait à Mozilla un numéro très en dessous de tout ce qui
    est déjà pris, dont le refus (« version already exists ») tombe APRÈS la
    validation, quota consommé ; et cela laisserait les deux manifests
    estampillés d'un numéro qui fait reculer l'extension de plusieurs majeurs,
    committé tel quel par le workflow. Deux dégâts pour une seule ligne
    illisible. On ne devine pas, on refuse.
    """
    # LE PLANCHER PORTE DÉJÀ UN QUATRIÈME NOMBRE dès la première ressortie, et
    # la grammaire du projet n'en connaît que trois : lu tel quel, « 1.0.0.1 »
    # passerait pour illisible et l'outil REFUSERAIT de choisir un numéro. Ce
    # repli sert justement quand la cible ne convient pas ; le voir tomber là
    # bloquerait la seule issue qui reste. On monte le tronc, et le compteur de
    ...
    try:
        ...
        # de ces outils ne sait relire. Laissée remonter, l'exception sortirait
        # en trace d'exécution au milieu du journal du run : illisible, et sans
        # le seul mot qui compte, à savoir qu'aucune signature n'a été tentée.
...
def stamp_version(version):
    ...
    C'EST AUSSI LA SEULE PORTE PAR OÙ UN NUMÉRO ENTRE, tout court. Les manifests
    ne se montent jamais à la main : le numéro se calcule au-dessus de ce qui est
    déjà pris ici comme chez Mozilla, et une retouche manuelle serait soit
    écrasée sans être lue, soit refusée après validation, quota consommé.
    """
    ...
    # (« 1.0.0.1 ») serait déclarée illisible et rien ne partirait chez Mozilla.
...
def parties_connues(state):
    ...
    TOUT EST FACULTATIF ICI, et rien ne doit tomber. Une entrée absente,
...
def texte_versions_parties(parties):
    ...
    return (
        ...
        " * quoi une signature faite depuis la beta brûlerait un numéro que la\n"
...
def parse_ver(v):
    ...
    Un repli SILENCIEUX qui rendrait (0,) sur tout ce qu'il ne sait pas lire
    ferait passer une version suffixée pour la plus basse de toutes : le
    plancher serait mal choisi, et l'outil pourrait proposer un recul à Mozilla,
    refusé en fin de pipeline, après la validation. Il parle.
    """
    r = V.rang(str(v))
    if r is not None:
        return r
    try:
        # les numéros à quatre nombres (« 1.0.0.1 », le compteur de signatures)
        # se comparent ici, complétés à trois nombres au minimum, jamais tronqués
...
def signature_suivante(base, cible):
    ...
    « 1.0.0.0 » pour ÉGAL à « 1.0.0 ». Soumettre le second après le premier
    serait refusé APRÈS la validation, quota du jour consommé pour rien. La
    première ressortie est donc « .1 » — et c'est aussi pour cela que le tout
    premier envoi, depuis des manifests en 1.0.0.0, part en 1.0.0.1.
    ...
    # quel rendrait « illisible » et ferait tomber l'outil dans le refus. On le
...
def numero_a_poser(base, cible):
    ...
    prend le quatrième nombre, et à défaut on monte d'un cran Z au-dessus du
    plancher et on le DIT fort. Mozilla refuse un numéro déjà pris, et son refus
    tombe APRÈS la validation, quota consommé : le seul cas où l'on peut se
    permettre d'être silencieux, c'est celui où l'on avance vraiment.
    """
    ...
    if cible:
        return (montee, [f"::warning title=Extension Outward décalée du projet::le site "
    ...
    return (montee, ["::warning title=Numéro du projet illisible::"
                     "docs/owd-manifeste.json ne donne pas de release lisible : "
...
def version_du_projet():
    ...
    C'est docs/owd-manifeste.json qui fait foi : c'est lui que lisent l'amorceur
...
def amo_latest(guid, issuer, secret):
    ...
    l'empreinte locale et ce qu'AMO connaît déjà. Au tout premier envoi, ce GUID
    n'existe pas encore chez Mozilla et cette fonction rend None : c'est normal.
    """
    try:
...
def xpi_signe():
    """True si docs/download/owd-roll20-firefox.xpi est un binaire signé Mozilla."""
    import zipfile
    try:
...
def repare_xpi_signe(issuer, secret):
    """Garde-fou : un commit local peut écraser le .xpi signé par un pack de
    développement NON signé — le site distribue alors un fichier que Firefox
    refuse d'installer, sans que rien n'échoue en CI. Si le .xpi committé n'est
    pas signé, on re-télécharge le dernier binaire signé depuis AMO (les GET ne
    subissent pas le quota de soumission) et on réaligne updates.json ; le
    commit de retour du workflow les republie.

    Tant que rien n'a jamais été signé, il n'y a rien à réparer et rien à dire
    de plus : la fonction sort sur son message, sans faire échouer le run."""
    if xpi_signe():
        return
    ...
    try:
        ...
        for v in r.json().get("results", []):
            ...
            # l'écrit aussi après une vraie signature : recopiée ici, les deux
            # copies ne se verraient pas
...
def main():
    ...
    #
    # build_extension.verifie() est le SEUL garde-fou des deux moitiés : elles
    # s'écrivent à la main, et il exige qu'elles ne diffèrent que par leurs
    # lignes marquées « propre à cette copie », et que chaque ligne marquée
    # nomme bien SA partie. Lancé seulement depuis la ligne de commande de
    # build_extension.py, il ne verrait jamais ce qui passe ICI — or c'est ici
    # que passe la CI, et build() emballe sans rien vérifier. Une moitié
    # retouchée d'un seul côté partirait donc chez Mozilla en silence.
    #
    # Avant l'empreinte, avant l'emballage, avant la moindre requête : un refus
    # ne doit rien coûter au quota, qui se compte à la dizaine par jour et qui
    # est partagé avec l'extension de la branche sœur.
    if not build_extension.verifie():
        sys.exit("[ci-extension] contrôles du paquet en échec : rien n'est signé")

    # PAS D'EXTENSION, PAS DE SIGNATURE. verifie() laisse passer l'absence
    # totale de paquet (voir build_extension.paquet_absent : « rien » est un
    # état, « à moitié » est une faute) ; ici, en revanche, on est venu SIGNER,
    # et il n'y a rien à signer. Le dire plutôt que d'aller s'écraser sur un
    # manifeste introuvable à la première lecture.
    if build_extension.paquet_absent():
        sys.exit("[ci-extension] il n'y a pas encore d'extension sous extension/ : "
                 "rien à empaqueter, rien à signer.")
    ...
    if current == state.get("hash"):
        ...
        # rien à synchroniser depuis le site (l'extension est une coquille) : on
        # remet juste les archives committées à l'exact, au cas où un pack de
        # développement local les aurait écrasées
        subprocess.run(["git", "checkout", "--",
                        "docs/download/owd-roll20-firefox.xpi",
                        "docs/download/owd-roll20-chrome.zip"],
    ...
    try:
        ...
        # n'est pas écrite, la prochaine relance manuelle retentera la
        # signature.
        subprocess.run(["git", "checkout", "--",
                        "docs/download/owd-roll20-firefox.xpi",
                        "docs/download/owd-roll20-chrome.zip",
        ...
        if "quota" not in str(e) and amo_latest(guid, issuer, secret) is None:
            print("::error title=Signature Outward impossible::" + str(e) +
            ...
            print(f"::warning title=Extension Outward non signée::{e} — le site "
