"""Signe l'extension Firefox MIA via l'API de soumission de Mozilla (AMO).
...
en PUT répond désormais 404 pour les nouveaux add-ons) :
  ...
  4. attente de la signature (file.status == "public"), téléchargement du .xpi
     signé PAR-DESSUS docs/download/mia-roll20-firefox.xpi ;
...
XPI = ROOT / "docs" / "download" / "mia-roll20-firefox.xpi"
UPDATES = ROOT / "docs" / "download" / "updates.json"
XPI_URL = "https://igneefleur.github.io/fns-ttrpg/mia/download/mia-roll20-firefox.xpi"
    ...
    for _ in range(4):
        with XPI.open("rb") as fh:
            r = amo.post("/addons/upload/",
                         files={"upload": ("mia-roll20-firefox.xpi", fh, "application/x-xpinstall")},
