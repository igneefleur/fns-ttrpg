...
(function (root) {
  ...
  var SCALARS = [
    ...
    // la règle de l'écart, coupée pour ce personnage
    ["ecartCoupe", "ecart_coupe", "b"],
  ...
  var COLLECTIONS = [
    ...
    ["desavantages", "desavantages"], ["attaques", "attaques"],
    ...
    ["caracs", "caracs"],
    // LE BONUS de chaque caractéristique (réglé sur la fiche) et LES CINQ
    // LEVIERS du meneur, qui règlent ce qu'elle donne sans toucher la valeur.
    ["caracsBonus", "caracs_bonus"], ["compsBonus", "comps_bonus"],
    // LES CINQ LEVIERS EN UN SEUL ATTRIBUT. Ils étaient huit collections à plat
    // (caracs_mod_mod, caracs_lim_mod, caracs_ecart, les trois de l'xp, les deux
    // du plafond) ; le schéma 2 les range dans une seule table à deux niveaux —
    // levier, puis boîte de la chaîne. Une collection voyage en BLOC, sans
    // jamais être ouverte : la profondeur ne coûte rien ici, et c'est huit
    // attributs de moins sur un personnage. Les huit anciens se LISENT encore,
    // plus bas (COLLECTIONS_HERITAGE).
    ["caracsLeviers", "caracs_leviers"],
    // LES COMPÉTENCES, sigle -> points investis. Mêmes leviers, mêmes raisons :
    // les six tables à plat du schéma 2 se sont rangées dans une seule au
    // schéma 3, et se LISENT encore plus bas (COLLECTIONS_HERITAGE).
    ["comps", "competences"],
    ["compsLeviers", "comps_leviers"],
    // CE QUE LES RÈGLES DISENT, ET QUE LE MENEUR PEUT DIRE AUTREMENT : la
    // caractéristique du jet par défaut, et celles qui commandent le plafond.
    // Seule la SURCHARGE voyage — la règle, elle, n'existe pas de ce côté-ci du
    // pont (DATA n'y est jamais chargé), et une copie figerait le personnage.
    ["compsCarac", "comps_carac"],
    ["compsCaracsPlafond", "comps_caracs_plafond"],
    ...
    // bonus, leviers }), donc tout tient dans ce seul attribut.
    ["specialites", "specialites"],
    // LES ARTS voyagent en BLOC comme toute collection : l'arbre
    // art > {base, ameliorations[]} > cinq champs tient dans un seul attribut,
    // et l'ouvrir pour en faire plusieurs recréerait à la main les huit
    // collections que les schémas 2 et 3 ont eu tant de mal à ranger.
    ["arts", "arts"],
    ["langues", "langues"],
    ["reservesLeviers", "reserves_leviers"],
    // le multiplicateur de récupération, une réserve à la fois
    ["recupMulti", "recup_multi"],
  ...
  // CE QUE LA 1.13 A ÉCRIT, ET QU'ON NE RÉÉCRIRA PLUS. Ces huit collections ont
  // été rangées dans state.caracsLeviers au schéma 2. On ne les ÉCRIT plus — ce
  // serait dire deux fois la même chose — mais on les LIT encore : un
  // personnage enregistré par la 1.13 dont le mia_state a été vidé (l'attribut
  // pèse des centaines de kilo-octets, et des joueurs le vident) n'a plus
  // qu'elles, et le pas de migration du bundle saura les déplacer une fois
  // qu'il les aura retrouvées à la racine.
  var COLLECTIONS_HERITAGE = [
    ["caracsModMod", "caracs_mod_mod"], ["caracsLimMod", "caracs_lim_mod"],
    ["caracsEcart", "caracs_ecart"], ["caracsXpForce", "caracs_xp_force"],
    ...
    // et les six des compétences, rangées au schéma 3
    ...
    // et la table « divers », rangée dans la chaîne au schéma 8
    ["divers", "divers"]
  ];
  ...
  function blank() {
    return {
      ...
      // LE PRESTIGE, qui pose le plafond de chaque caractéristique — plafond
      // que le levier « plafond » relève ou remplace, une par une.
      ...
      caracs: {}, caracsBonus: {},
      // LES CINQ LEVIERS, une table à deux niveaux et une seule clé. Vide, comme
      // les autres, et pour la même raison : ses clés du second niveau sont les
      // sigles des règles, que seul DATA connaît.
      caracsLeviers: {},
      ecartCoupe: false,

      comps: {}, compsBonus: {},
      // miroir exact de 040-etat-defaut.js : la surcharge seule, et les leviers
      // dans une table à deux niveaux
      compsCarac: {}, compsCaracsPlafond: {}, compsLeviers: {},

      specialites: [], arts: [], langues: [], reservesLeviers: {},
  ...
  function reconstruire(cur) {
    ...
    // ON REPOSE LES HUIT ANCIENNES À LA RACINE, et on ramène l'état au schéma 1
    // pour que le bundle les migre. Sans la seconde moitié, blank() aurait
    // estampillé le schéma 2 : le pas ne tournerait pas, et huit leviers
    // remplis disparaîtraient de la fiche sans un mot. La présence d'une clé
    // d'héritage suffit à dire d'où vient ce personnage.
    var herite = false;
    COLLECTIONS_HERITAGE.forEach(function (d) {
      var v = cur(d[1]);
      if (v === undefined || v === "") return;
