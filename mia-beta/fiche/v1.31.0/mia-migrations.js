...
(function (global) {
  ...
  // ---------- 6 -> 7 : le nom de la spécialité de récupération ----------
  //
  //     spécialité « RÉCUP » ou « Récupération »  ->  « RÉCUP PV »
  //
  // UN NOM, ET RIEN D'AUTRE. Les points ne bougent pas, ni les leviers, ni le
  // rang dans la liste : seule la chaîne du nom change. Mais elle change une
  // DONNÉE EXISTANTE, et c'est la définition même d'un pas de schéma.
  //
  // POURQUOI UN PAS PLUTÔT QU'UN ALIAS DE PLUS. Le moteur sait déjà lire les
  // trois noms — RECUP_NOMS, dans 090-calculs-corps.js — et rien n'aurait
  // cassé sans ce pas. Mais le bloc Vitalité affiche le nom CANONIQUE tandis
  // que la liste des Spécialités affiche celui de l'état : sans renommage, la
  // même spécialité se serait lue « RÉCUP PV » à un endroit et « RÉCUP » à
  // deux lignes de là. Les alias restent pour les fiches qu'un pas n'a pas
  // encore touchées (le repli Roll20, une archive rouverte).
  //
  // LA PREMIÈRE SEULE. speParNom() rend la première de la liste et le reste est
  // invisible au moteur : renommer les deux fabriquerait deux « RÉCUP PV »,
  // dont l'une continuerait de coûter de l'xp sans rien produire.
  var RECUP_7 = ["récup", "récupération"];
  function recupIndex(s) {
    var l = Array.isArray(s.specialites) ? s.specialites : [], i, n;
    for (i = 0; i < l.length; i++) {
      n = estTable(l[i]) ? String(l[i].nom == null ? "" : l[i].nom).trim().toLowerCase() : "";
      if (RECUP_7.indexOf(n) >= 0) return i;
    }
    return -1;
  }
  ...
  MiaMigr.ajouter({
    schema: 7,
    titre: "Le nom de la spécialité de récupération",
    notes: "La spécialité qui commande la récupération s'appelle désormais "
         + "« RÉCUP PV ». Ses points, ses leviers et sa place dans la liste ne "
         + "bougent pas : seul son nom change, pour que la Fiche et le bloc "
         + "Vitalité la nomment pareil. Aucun chiffre ne change.",
    // LE CASIER NE PORTE QU'UNE SEULE CHOSE : le nom à rendre à la descente
    // suivante. Trois états, un seul sens :
    //     absent   — personne n'est passé ;
    //     la chaîne — le nom que la fiche portait vraiment au schéma 6 ;
    //     null     — la descente a écrit ce nom elle-même, il n'était pas d'ici.
    // Le pas 6 avait fait porter DEUX notes de nature différente au même casier,
    // et la seconde ne s'écrivait jamais : on ne le refait pas.
    monter: function (s, ctx) {
      var reprise = ctx.reprendre("recup7");
      var i = recupIndex(s);
      if (i >= 0) {
        // QUI REVIENT NE RANGE RIEN. « reprise » vaut null quand la descente a
        // écrit ce nom : sans cette distinction, 7 -> 6 -> 7 rendait un grenier
        // que l'état d'origine n'avait pas.
        if (reprise === undefined) ctx.grenier("recup7", String(s.specialites[i].nom));
        s.specialites[i].nom = "RÉCUP PV";
      }
      return s;
    },
    descendre: function (s, ctx) {
      var repris = ctx.reprendre("recup7");
      var l = Array.isArray(s.specialites) ? s.specialites : [], i;
      for (i = 0; i < l.length; i++) {
        if (!estTable(l[i])) continue;
        if (String(l[i].nom == null ? "" : l[i].nom).trim().toLowerCase() !== "récup pv") continue;
        // LE NOM D'AVANT S'IL EST CONNU, sinon celui du schéma 6. Une fiche
        // née au 7 n'a jamais porté d'ancien nom : elle redescend sous le nom
        // que le 6 savait lire, et l'on SIGNE ce nom-là pour que la remontée ne
        // le prenne pas pour le sien.
        if (typeof repris === "string") l[i].nom = repris;
        else { l[i].nom = "RÉCUP"; ctx.grenier("recup7", null); }
        break;
      }
      return s;
    }
  ...
  // ---------- 7 -> 8 : toutes les valeurs dérivées passent à la chaîne ----------
  //
  //     <x>Override        ->  reservesLeviers.<levier>.force
  //     divers.<x>[0,1,2]  ->  reservesLeviers.<levier>.a1, a2, a3
  //
  // LE MÊME PAS QUE LES 5 ET 6, SIX FOIS D'UN COUP. Les deux réserves étaient
  // passées les premières ; l'initiative, la récupération, la vitesse, la charge
  // et les deux sauts les suivent, et la table « divers » disparaît avec elles.
  //
  // LA CORRESPONDANCE EST EXACTE ET LES CHIFFRES NE BOUGENT PAS : les trois
  // modificateurs s'additionnaient à la base, et a1 + a2 + a3 s'y additionnent
  // de la même façon, avant les facteurs — qui valent un tant que personne n'y
  // touche. Une valeur forcée reste forcée.
  //
  // LE CASIER « d8 » NE PORTE QU'UN SENS : ce qu'il faut rendre à la descente.
  //     absent  — personne n'est passé ;
  //     l'objet — ce que la fiche portait vraiment au schéma 7 ;
  //     null    — la descente a fabriqué ces clés, elles n'étaient pas d'ici.
  // Le pas 6 avait fait porter deux notes de nature différente au même casier et
  // la seconde ne s'écrivait jamais : on ne le refait plus.
  var DERIVES_7_8 = [
    ["initiative", "initiativeOverride", "initiative"],
    ["recupJour",  "recupOverride",      "recup"],
    ["vitesse",    "vitesseOverride",    "vitesse"],
    ["charge",     "chargeOverride",     "charge"],
    ["sautLong",   "sautLongOverride",   "sautLong"],
    ["sautHaut",   "sautHautOverride",   "sautHaut"]
  ];
  var BOITES_7_8 = [["a1", 0], ["a2", 1], ["a3", 2]];
  function troisCases(k) { return k === "force" || k === "a1" || k === "a2" || k === "a3"; }

  MiaMigr.ajouter({
    schema: 8,
    titre: "Les valeurs dérivées passent à la chaîne",
    notes: "L'initiative, la récupération, la vitesse, la charge et les deux "
         + "sauts se règlent désormais dans l'onglet Options, avec la même "
         + "chaîne que les deux réserves : une valeur imposée, deux ajouts, deux "
         + "facteurs, deux ajouts, deux facteurs. Les valeurs forcées et les "
         + "trois modificateurs qui vivaient sur la Fiche y entrent tels quels, "
         + "et la table « divers » disparaît. Aucun chiffre ne change.",
    monter: function (s, ctx) {
      var repris = ctx.reprendre("d8");
      var reste = ctx.reprendre("d8reste");
      var avaitTable = Object.prototype.hasOwnProperty.call(s, "reservesLeviers");
      var lv = estTable(s.reservesLeviers) ? s.reservesLeviers : {};
      var etat = {};
      DERIVES_7_8.forEach(function (d) {
        var nom = d[0], ovr = d[1], div = d[2];
        var max = estTable(lv[nom]) ? lv[nom] : {};
        // ce que la descente avait mis de côté, faute de place dans trois cases
        var r = estTable(reste) && estTable(reste[nom]) ? reste[nom] : null;
        if (r) Object.keys(r).forEach(function (k) {
          if (max[k] === undefined) max[k] = r[k];
        });
        // le forçage
        if (typeof s[ovr] === "number" && isFinite(s[ovr]) && max.force === undefined) {
          max.force = s[ovr];
        }
        if (Object.prototype.hasOwnProperty.call(s, ovr)) etat[nom] = { force: true };
        delete s[ovr];
        // les trois modificateurs
        var dv = estTable(s.divers) ? s.divers : null;
        var tab = dv && Array.isArray(dv[div]) ? dv[div] : null;
        if (tab) {
          BOITES_7_8.forEach(function (p) {
            var v = tab[p[1]];
            // UN AJOUT DE ZÉRO NE PASSE PAS : il ne faisait rien là-bas et ferait
            // d'une boîte vide une boîte RÉGLÉE ici.
            if (typeof v !== "number" || !isFinite(v) || v === 0) return;
        ...
        if (Object.keys(max).length) lv[nom] = max;
      });
      // LA TABLE « divers » VIDÉE S'EN VA : le schéma 8 ne la connaît plus.
      if (estTable(s.divers) && !Object.keys(s.divers).length) delete s.divers;
      if (Object.keys(lv).length || avaitTable) s.reservesLeviers = lv;
      // QUI REVIENT NE RANGE RIEN : « repris » vaut null quand la descente a
      // fabriqué ces clés, et l'objet quand elle a rendu les vraies.
      if (repris === undefined && Object.keys(etat).length) ctx.grenier("d8", etat);
      return s;
    },
    descendre: function (s, ctx) {
      var repris = ctx.reprendre("d8");
      var avaitTable = Object.prototype.hasOwnProperty.call(s, "reservesLeviers");
      var lv = estTable(s.reservesLeviers) ? s.reservesLeviers : {};
      var reste = {}, signe = false;
      var divers = estTable(s.divers) ? s.divers : {};
      DERIVES_7_8.forEach(function (d) {
        var nom = d[0], ovr = d[1], div = d[2];
        var max = estTable(lv[nom]) ? lv[nom] : {};
        var rec = estTable(repris) && estTable(repris[nom]) ? repris[nom] : null;
        // ON ÉCRIT, ON N'EFFACE PAS : une clé qu'on n'a pas produite reste.
        if (typeof max.force === "number" && isFinite(max.force)) {
          s[ovr] = max.force;
          signe = true;
        } else if (rec && rec.force) s[ovr] = null;
        var pose = false, tab = [0, 0, 0];
        BOITES_7_8.forEach(function (p) {
          var v = max[p[0]];
          if (typeof v === "number" && isFinite(v)) { tab[p[1]] = v; pose = true; }
        ...
        // LES BOÎTES QUE TROIS CASES NE SAVENT PAS PORTER vont au grenier : les
        // deux ajouts de fin et les quatre facteurs.
        var r = {};
        Object.keys(max).forEach(function (k) { if (!troisCases(k)) r[k] = max[k]; });
        if (Object.keys(r).length) reste[nom] = r;
        delete lv[nom];
      });
      if (Object.keys(reste).length) ctx.grenier("d8reste", reste);
      if (Object.keys(divers).length) s.divers = divers;
      // LA TABLE COMMUNE RESTE : le schéma 7 la connaît, et « pvMax » comme
      // « enduranceMax » y vivent — ils appartiennent aux pas 5 et 6, pas à
      // celui-ci. On ne retire que NOS clés.
      if (avaitTable || Object.keys(lv).length) s.reservesLeviers = lv;
      else delete s.reservesLeviers;
      // ET L'ON NE SIGNE QUE CE QU'ON A ÉCRIT.
      if (repris === undefined && signe) ctx.grenier("d8", null);
      return s;
    }
  });
