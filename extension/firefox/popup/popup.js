 ...
 *   owdOff             l'interrupteur général      (absent = allumée)
 *   owdNuit            "auto" | "jour" | "nuit"    (absent = auto)
 *   owdPanneauActif    le panneau de Camp          (absent = allumé)
 *   owdBeta            la moitié beta              (absent = stable)
 *
 * UN SEUL NOM PAR RÉGLAGE, et c'est le luxe d'une extension neuve : rien n'est
 * encore posé chez un joueur, donc rien à ménager. Les deux copies de
 * content-roll20.js lisent exactement ces quatre clés-là, écrites exactement
 * comme ici. Le jour où une cinquième s'ajoutera, elle s'écrira des deux côtés
 * dans le même geste : un nom écrit d'un seul côté, c'est un interrupteur qui
 * ne fait plus rien, sans le moindre message, et personne ne s'en aperçoit
 * avant une vraie partie.
 *
 * Aucune collision avec la géométrie du panneau : celle-ci est rangée sous
 * « owdPanneau:roll20-camp.html », et storage.local.get("owdPanneauActif") ne
 * rend que la clé exacte, jamais ce qui commence pareil.
 *
 * LE MOT EST « BETA » DANS TOUT CE QU'UN JOUEUR LIT — le réglage, la pastille et
 * les lignes du pied. Il n'y en a pas de second, et il ne faut pas en inventer.
 *
 * CE QU'IL NE FAIT PAS : repeindre quoi que ce soit selon le mode. Une interface
 * qui change de couleur selon la moitié chargée fait douter de ce qu'on lit, et
 * la couleur ne porterait aucune information que le mot ne porte mieux. Il ne
 * reste du mode qu'une pastille de texte et deux adresses de lien qui changent.
 *
 * CE QU'IL N'ÉCRIT PAS : de notice sous les réglages. Une phrase qui redit le
 * libellé, ou qui annonce « prend effet au rechargement » alors que le bouton de
 * rechargement sort tout seul quand c'est vrai, n'apprend rien. Ce qui ne se
 * devine pas est en title=, le reste n'est pas écrit.
 *
...
(function () {
  ...
  var CLE_OFF = "owdOff";
  var CLE_NUIT = "owdNuit";
  var CLE_BETA = "owdBeta";
  var CLE_PAN = "owdPanneauActif";
  // La géométrie du panneau : posée par content-roll20.js, jamais par le popup,
  // qui ne fait que la SUPPRIMER quand on demande à replacer.
  var CLE_PAN_GEO = "owdPanneau:roll20-camp.html";

  var CLE_DEP = ["owd_sheet_url", "owd_site_url"];
  ...
  // rendu ; owdNuit reste la seule autorité, et les deux se recalent dès que le
  // stockage a répondu.
  var CLE_ECHO = "owdNuitEcho";
  ...
  // Les adresses sont écrites EN TOUTES LETTRES, jamais assemblées : concaténées,
  // elles deviendraient invisibles à l'analyse statique d'AMO comme au contrôle
  // de complétude, qui ne savent lire que des littéraux. Et toutes en minuscules,
  // GitHub Pages étant sensible à la casse — un chemin en capitales répond 404.
  var REGLES = {
    stable: "https://igneefleur.github.io/fns-ttrpg/owd/content/regles/",
    beta: "https://igneefleur.github.io/fns-ttrpg/owd-beta/content/regles/"
  };
  var PAGE_EXT = {
    stable: "https://igneefleur.github.io/fns-ttrpg/owd/extension/",
    beta: "https://igneefleur.github.io/fns-ttrpg/owd-beta/extension/"
  };
  // LES NUMÉROS DES FICHES SE LISENT À LA SOURCE, jamais en dur.
  //
  // Figés dans le paquet signé, ils deviendraient faux à la première mise à jour
  // du site et le resteraient jusqu'à la signature suivante : une fiche avance
  // sans qu'on signe quoi que ce soit. Le popup va donc les chercher dans les
  // deux manifestes de site, qui sont, eux, republiés à chaque déploiement.
  //
  ...
  // origine passe. Élargir les permissions ferait réexaminer l'extension et
  // redemanderait leur accord aux joueurs, pour deux nombres d'affichage.
  var MANIFESTES = {
    stable: "https://igneefleur.github.io/fns-ttrpg/owd/owd-manifeste.json",
    beta: "https://igneefleur.github.io/fns-ttrpg/owd-beta/owd-manifeste.json"
  };
  ...
  function demarre() {
    ...
    function rendPieces() {
      ...
      // rien d'autre : au clavier, ces trois boutons resteraient dans l'ordre de
      // tabulation et se déclencheraient à la barre d'espace, sur un bloc que
    ...
    function rendDepannage() {
      ...
      // afficher le site stable sous un onglet « Fiche Outward beta », et rien
      // nulle part ne l'annonce : ce panneau est le seul endroit qui puisse le
      // dire sans ouvrir une console.
      var mots = [], i;
    ...
    // selon ce que l'outil de signature écrit dans parties.js — le numéro nu ou
    // déjà suffixé — on lirait sinon une ligne sans suffixe, ou « 1.0.0b b ». Les
    // deux conventions donnent ici le même affichage.
    ...
    function rendVersions() {
      ...
      // encore à sa valeur par défaut désignerait la stable, puis la marque
      // sauterait sur la beta un instant plus tard. Un clignotement qui dit deux
      // vérités.
      //
    ...
    //
    // ET ON NE TÉLÉPHONE PAS QUAND L'EXTENSION EST ÉTEINTE : une extension
    // arrêtée qui va quand même chercher deux nombres sur un site n'est pas une
    // extension arrêtée.
    ...
    // permission de plus ferait réexaminer l'extension et redemanderait leur
    // accord aux joueurs, pour un bouton de confort.
    //
    // browser.tabs.reload() est appelé SANS ARGUMENT : ainsi il recharge
    // l'onglet actif de la fenêtre courante et ne demande PAS la permission
    // tabs. Le manifeste n'en déclare qu'une, storage, et c'est un invariant.
    ...
    function choisitNuit(v) {
      ...
      pose({ owdNuit: v });
      rendNuit();
      // Le panneau s'habille seul, mais la fiche et le camp d'une partie déjà
    ...
    elPan.addEventListener("click", function () {
      ...
      touche[CLE_PAN] = true;
      // Clé écrite par son nom de variable : la même chaîne que celle que les
      // deux copies de content-roll20.js lisent, et une seule.
      var o = {};
      o[CLE_PAN] = etat.panneau;
      pose(o);
      rendPieces();
      // Pas de proposition de rechargement ici : le panneau se monte et se
      // démonte à l'ouverture d'une partie, et l'interrupteur ne vaut que pour
      // la suivante. Un bouton qui sortirait pour tout ferait recharger pour
      // rien.
    });

    elPanReplacer.addEventListener("click", function () {
      // Traîné hors de l'écran ou replié dans un coin, le panneau ne se
      // récupérerait par aucun chemin visible : effacer sa géométrie lui rend sa
    ...
    // stockage en panne laisserait des cases dans leur état par défaut sans que
    // rien ne le dise ; ici l'échec passe par le même chemin que la réponse
    // vide, et le panneau s'affiche avec les valeurs par défaut, qui sont aussi
    // celles que les scripts de contenu prendront.
    var voulues = [CLE_OFF, CLE_NUIT, CLE_BETA, CLE_PAN].concat(CLE_DEP);
    try {
    ...
    function recu(r) {
      ...
      // Absent = allumé : une partie Roll20 qui n'a rien à voir avec Outward
      // doit pouvoir se débarrasser du panneau sans désinstaller, mais ne doit
      // pas avoir à l'allumer pour l'avoir. Le test est donc STRICT sur false,
      // et non « r[CLE_PAN] !== undefined ».
      if (!touche[CLE_PAN]) {
        etat.panneau = r[CLE_PAN] !== false;
      }
