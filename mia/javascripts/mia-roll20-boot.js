/* Amorce Roll20 de la fiche MIA — chargée UNIQUEMENT par roll20-fiche.html,
 * la page que l'extension Roll20 affiche dans son iframe.
 *
 * Architecture « coquille » : l'extension signée ne contient plus la fiche.
 * Elle pose l'onglet « Fiche MIA », le pont d20 (Attributes) et le relais
 ...
 *  1. Poser window.__miaLocalStorage : un shim SYNCHRONE adossé à un cache
 *     mémoire. mia-creation.js (via STORE) persiste dedans à la place du
 *     localStorage réel.
 *  2. Demander au pont d20 les Attributes du perso (type "load", relancé tant
 *     que le pont n'a pas répondu) ; à la réception ("hydrate"),
 *     MiaAttrMap.attrsToState() reconstruit l'état -> cache sous « mia-perso ».
 *  3. SEULEMENT ALORS injecter javascripts/mia-creation.js : son init()/load()
 *     lit l'état déjà hydraté et monte la fiche.
 *  4. À chaque sauvegarde (setItem « mia-perso »/« mia-cards »),
 *     MiaAttrMap.stateToAttrs() redécompose l'état ; seuls les attributs
 ...
 *  6. GEL. attrsToState() rend son diagnostic avec l'état. Un mia_state présent
 ...
 *     mia_state VIDÉ chez un personnage qui garde ses autres attributs mia_
 ...
 * soient les origines intermédiaires. Messages tagués ns:"mia" + charId.
 */
(function () {
  "use strict";
  var M = window.MiaAttrMap;
  ...
  var SAVE_KEYS = { "mia-perso": 1, "mia-cards": 1 };
  ...
  var taillesPostees = [];      // tailles récentes de mia_state posté (repère du chien de garde)
  var gele = false;             // fiche lue à moitié : on n'écrit plus rien (voir hydrate)

  window.__miaLocalStorage = {
  ...
  function post(msg) { msg.ns = "mia"; msg.charId = CHAR_ID; try { window.top.postMessage(msg, "*"); } catch (e) {} }

  // ---------- mode jour / nuit ----------
  // Le CSS nuit existe déjà (mia-creation.css : html.night .perso-atelier) ; ici
  ...
  // L'onglet Options de la fiche expose ce réglage via window.__miaNight.
  var NIGHT_KEY = "mia-r20-night";
  ...
  // Signale à mia-creation.js qu'on est dans Roll20 : affichage condensé
  // « fiche » et jets envoyés au TCHAT Roll20 (au lieu du journal local).
  window.__miaCompact = true;
  window.__miaRoll = function (die, value, label) { post({ type: "roll", die: die, value: value, label: label }); };
  // envoi d'un ÉLÉMENT de la fiche (passif, arme, avantage…) au tchat Roll20
  window.__miaSay = function (title, fields) { post({ type: "say", title: title, fields: fields }); };
  ...
  window.__miaChat = function (raw) { post({ type: "chat", raw: raw }); };
  // Objet pris au tchat : l'extension relaie le payload du lien. Il peut
  // arriver AVANT que mia-creation.js soit monté (clic pendant le chargement) :
  // on le met alors en attente et on le rejoue dès que la fiche répond.
  var takeQueue = [];
  window.__miaTake = function (payload) {
    if (typeof window.__miaOnTake === "function") { window.__miaOnTake(payload); return; }
  ...
  var playersWait = [];
  window.__miaPlayers = function (cb) {
  ...
  setInterval(function () {
    if (!takeQueue.length || typeof window.__miaOnTake !== "function") return;
    var q = takeQueue.slice();
    takeQueue.length = 0;
    q.forEach(function (p) { window.__miaOnTake(p); });
  }, 400);
  ...
  function doSave() {
    saveTimer = null;
    var state;
    try { state = JSON.parse(mem["mia-perso"] || "null"); } catch (e) { return; }
    if (!state) return;
    var card = null;
    try { var cards = JSON.parse(mem["mia-cards"] || "{}"); card = cards && cards._current; } catch (e) {}
    ...
    // repère du chien de garde : la taille du mia_state qu'on vient de poster
  ...
  // mia_state pèse couramment des centaines de kilo-octets ; le comparer en
  ...
  // ---------- bandeau ----------
  // Les styles sont injectés d'ici et non posés dans stylesheets/mia-roll20.css :
  ...
  var CSS_BANDEAU =
    "#mia-bandeau{position:sticky;top:0;z-index:50;display:flex;flex-wrap:wrap;align-items:center;" +
    ...
    "#mia-bandeau .mia-bandeau-txt{flex:1 1 14rem;min-width:0}" +
    "#mia-bandeau .mia-bandeau-btn{flex:0 0 auto;padding:.2rem .6rem;border:1px solid #c9a97c;" +
    "border-radius:.2rem;background:#fffaf0;color:#4c3a24;font:inherit;cursor:pointer}" +
    "#mia-bandeau .mia-bandeau-btn:hover{background:#fff}" +
    "html.night #mia-bandeau{background:#3a2c1c;color:#e8dcc6;border-bottom-color:#6b5636}" +
    "html.night #mia-bandeau .mia-bandeau-btn{background:#4a3a26;color:#e8dcc6;border-color:#6b5636}" +
    ...
    "#mia-ecran{position:fixed;top:0;left:0;right:0;bottom:0;z-index:80;overflow:auto;" +
    ...
    "#mia-ecran .mia-ecran-boite{box-sizing:border-box;max-width:34rem;margin:0 auto;background:#fffaf0;" +
    "border:1px solid #c9a97c;border-radius:.3rem;padding:.9rem 1rem}" +
    "#mia-ecran.mia-ecran-rouge .mia-ecran-boite{border-color:#a5342c;background:#fff3f0}" +
    "#mia-ecran h1{margin:0 0 .5rem;font-family:'Cinzel',serif;font-size:1.15rem;line-height:1.25}" +
    "#mia-ecran h2{margin:.85rem 0 .25rem;font-size:.88rem}" +
    "#mia-ecran p{margin:.35rem 0}" +
    "#mia-ecran ul{margin:.3rem 0;padding-left:1.1rem}" +
    "#mia-ecran li{margin:.25rem 0}" +
    "#mia-ecran .mia-ecran-vers{display:flex;flex-wrap:wrap;gap:.2rem 1.4rem;margin:.5rem 0;" +
    "padding:.35rem .6rem;background:#f6e2c8;border-radius:.2rem}" +
    "#mia-ecran .mia-ecran-actions{display:flex;flex-wrap:wrap;gap:.5rem;margin:.85rem 0 .4rem}" +
    "#mia-ecran button{padding:.35rem .7rem;border:1px solid #c9a97c;border-radius:.2rem;" +
    "background:#fffaf0;color:#4c3a24;font:inherit;cursor:pointer}" +
    "#mia-ecran button:hover:not([disabled]){background:#fff}" +
    "#mia-ecran button.mia-ecran-primaire{background:#4c3a24;color:#fffaf0;border-color:#4c3a24}" +
    "#mia-ecran button[disabled]{opacity:.45;cursor:not-allowed}" +
    "#mia-ecran .mia-ecran-note{font-size:.72rem;opacity:.85}" +
    "#mia-ecran label.mia-ecran-epingle{display:block;margin:.2rem 0}" +
    ...
    "#mia-ecran textarea{box-sizing:border-box;width:100%;min-height:8rem;margin-top:.3rem;" +
    "font-family:'Roboto Mono',monospace;font-size:.66rem}" +
    "html.night #mia-ecran{background:#241b12;color:#e8dcc6}" +
    "html.night #mia-ecran .mia-ecran-boite{background:#33271a;border-color:#6b5636}" +
    "html.night #mia-ecran.mia-ecran-rouge .mia-ecran-boite{background:#3a231f;border-color:#a5342c}" +
    "html.night #mia-ecran .mia-ecran-vers{background:#4a3a26}" +
    "html.night #mia-ecran button{background:#4a3a26;color:#e8dcc6;border-color:#6b5636}" +
    "html.night #mia-ecran button.mia-ecran-primaire{background:#e8dcc6;color:#33271a;border-color:#e8dcc6}" +
    "html.night #mia-ecran textarea{background:#241b12;color:#e8dcc6;border-color:#6b5636}";
  function poserStyles() {
    if (document.getElementById("mia-bandeau-css")) return;
    var s = document.createElement("style");
    s.id = "mia-bandeau-css";
  ...
  // `acte` est un nom stable posé en data-mia-act : les libellés se réécrivent,
  // les harnais de test s'accrochent à ça et non à la prose.
  function bandeau(txt, actions) {
    poserStyles();
    var b = document.getElementById("mia-bandeau");
    if (!b) {
    ...
    p.className = "mia-bandeau-txt";
    p.textContent = txt;
    b.appendChild(p);
    (actions || []).forEach(function (a) {
      var btn = document.createElement("button");
      btn.type = "button";
      btn.className = "mia-bandeau-btn";
      btn.textContent = a.texte;
      if (a.acte) btn.setAttribute("data-mia-act", a.acte);
  ...
  // mia_state est là mais ne se lit pas : un caractère tapé dans l'onglet
  ...
  function bandeauGelIllisible(raison) {
    bandeauGel("Cette fiche n'a pas pu être lue en entier (" + (raison || "état illisible") + "). " +
               ...
               "pas écraser ce que Roll20 contient encore. Récupérer l'attribut mia_state du " +
               "personnage avant toute chose ; il porte la fiche entière.");
  }
  // mia_state a disparu alors que le reste de la fiche est là. Rien n'est
  ...
  function bandeauGelDisparu() {
    bandeauGel("La fiche entière de ce personnage est introuvable : l'attribut mia_state, qui la " +
               "porte en un seul morceau, est vide, alors que les autres attributs mia_ sont " +
               ...
               "reconstruction. Si mia_state a été vidé par erreur, y remettre son contenu puis " +
  ...
  var PIN_PREFIX = "mia-r20-version:";
  ...
  function manifeste() { return window.__miaManifeste || null; }
  ...
  function specCourant() {
    ...
    return {
      js: (b && listeSure(b.js)) ? b.js : ["javascripts/mia-fiche.js"],
  ...
  function archiveDe(rel) {
    ...
    // celle que la carte d'attributs écrira dans mia_version.max. C'est la CLÉ
  ...
  //
  // C'est mia_state qui fait foi : lui seul est réécrit par la version qui a
  // réellement enregistré la fiche. Le max de mia_version, lui, porte la
  ...
  // (état d'abord, mia_version en repli quand l'état ne se lit plus) sans
  // reconstruire l'état, ce qui reste bon marché sur un mia_state énorme.
  ...
  function fermerEcran() {
    var e = document.getElementById("mia-ecran");
  ...
  function ecran(titre, rouge) {
    ...
    fond.id = "mia-ecran";
    if (rouge) fond.className = "mia-ecran-rouge";
    fond.setAttribute("role", "dialog");
    var boite = noeud("div", "mia-ecran-boite");
  ...
  function zoneActions(boite) {
    var d = noeud("div", "mia-ecran-actions");
    boite.appendChild(d);
    return d;
  }
  // acte : nom stable en data-mia-act, sur lequel s'accrochent les harnais.
  function bouton(zone, texte, acte, action, primaire) {
    var b = noeud("button", primaire ? "mia-ecran-primaire" : null, texte);
    b.type = "button";
    b.setAttribute("data-mia-act", acte);
    b.onclick = action;
    zone.appendChild(b);
    return b;
  }
  function griser(btn, raison, boite) {
    btn.disabled = true;
    btn.title = raison;
    boite.appendChild(noeud("p", "mia-ecran-note", raison));
  }
  ...
  function jsonPerso() {
    var brut = mem["mia-perso"] || "{}";
    try { return JSON.stringify(JSON.parse(brut), null, 2); } catch (e) { return brut; }
  }
  function nomFichier() {
    var n = "";
    try { n = (JSON.parse(mem["mia-perso"] || "{}") || {}).name || ""; } catch (e) {}
    return (n || "personnage-mia") + ".json";
  }
  ...
  function montrerJson(txt, intro) {
    var b = boiteCourante;
    if (!b) return;
    var zone = document.getElementById("mia-ecran-json-zone");
    if (!zone) {
      zone = noeud("div");
      zone.id = "mia-ecran-json-zone";
      b.appendChild(zone);
    }
    zone.innerHTML = "";
    zone.appendChild(noeud("p", "mia-ecran-note", intro));
    var ta = noeud("textarea");
    ta.id = "mia-ecran-json";
    ta.readOnly = true;
    ta.value = txt;
    zone.appendChild(ta);
  ...
  function chargerBundle(spec, remplacerCss) {
    ...
    bundleCharge = true;
    fermerEcran();
    // data : le mia-creation.json à lire. Une archive embarque le sien, gelé à
    // sa date ; sans lui un ancien bundle lirait les règles d'aujourd'hui.
    window.__miaDataUrl = spec.data || null;
    // RELEASE EFFECTIVE : celle qui TOURNE, pas celle que le site publie. Sans
    // ça, une archive enregistrerait mia_version.max à la version du site, la
    ...
    if (remplacerCss) {
      // Les feuilles du jour portent data-mia (posé par l'amorceur) : une
      // archive amène les siennes, et faire cohabiter les deux donnerait une
      // fiche à moitié restylée.
      var vieux = document.querySelectorAll("link[data-mia]");
  ...
  // poser window.MiaMigr, il ne touche à rien) ; s'il manque, l'écran le dit au
  // lieu d'inventer un « rien à signaler ».
  function avecMigrations(cb) {
    if (window.MiaMigr && window.MiaMigr.resume) { cb(); return; }
    var u = null;
    (specCourant().js || []).forEach(function (x) { if (!u && /mia-migrations/.test(x)) u = x; });
    if (!u) { cb(); return; }
    var fini = false;
  ...
  // mia_backup, on la RELIT dans le personnage, et seulement alors on injecte
  ...
  function lotBackup(f) {
    ...
    // Sans mia_state lisible, on met à l'abri la reconstruction champ par
    // champ : c'est exactement ce que la migration s'apprête à remplacer.
    var contenu = brut ? String(brut) : (mem["mia-perso"] || "");
    var lot = {};
    lot[M.PREFIX + BACKUP] = {
      current: contenu,
  ...
  // « Restaurer l'état d'origine » : relit mia_backup DANS LE PERSONNAGE (pas
  // en mémoire : c'est Roll20 qui fait foi) et le réécrit dans mia_state, avec
    ...
    sonde(function (recu) {
      ...
      if (!brut) {
        ...
        b0.appendChild(noeud("p", null,
          "L'attribut mia_backup du personnage est vide ou illisible. Rien n'a été touché ; la fiche reste " +
      ...
      // mia_version repart au schéma d'origine, sinon la fiche restaurée serait
      ...
      confirme(lot, function (ok) {
        if (!ok) {
          ...
          boutonExport(z1);
          montrerJson(brut, "L'état d'origine, tel qu'il était gardé dans mia_backup.");
          return;
        }
        lastAttrs = fusion(lastAttrs, lot);
        fermerEcran();
  ...
  function ecranVersion(f, relSite, schSite) {
    ...
    var v = noeud("div", "mia-ecran-vers");
    ...
    var pas = null;
    try {
      if (window.MiaMigr && window.MiaMigr.resume) pas = window.MiaMigr.resume(f.schema, schSite);
    } catch (e) { pas = null; }
    if (duFutur) {
      b.appendChild(noeud("p", null,
      ...
      pas.forEach(function (e) {
        var li = noeud("li");
        li.appendChild(noeud("span", "mia-ecran-pas", e.titre + " : "));
    ...
    var lab = noeud("label", "mia-ecran-epingle");
    var caseEp = document.createElement("input");
    caseEp.type = "checkbox";
    caseEp.id = "mia-ecran-epingle";
    ...
    b.appendChild(lab);
    b.appendChild(noeud("p", "mia-ecran-note",
  ...
  function decider() {
    ...
    // mia_state disparu, c'est ici que s'arrête le personnage qui n'a plus que
    ...
    // 1. personnage sans fiche MIA : rien à dire, la fiche du jour se monte.
    ...
    //
    // L'ACCORD LE SAUTE. /mia/ et /mia-beta/ sont servis par la même origine,
  ...
  // défaut de mia-attr-map font foi).
  ...
  function note(html) {
    var n = document.getElementById("mia-roll20-note");
  ...
  function hydrate(attrs) {
    ...
    //   null        -> mia_state lu, état complet ;
    //   "illisible" -> mia_state présent mais cassé ;
    //   "partiel"   -> pas de mia_state, et là tout dépend du RESTE.
    //
    // « partiel » couvre deux situations opposées, et le confondre en une
    // seule coûterait la fiche. Un personnage NEUF n'a aucun attribut mia_ :
    // il n'y a rien à perdre, on ouvre et on écrit normalement. Un personnage
    // qui garde ses autres attributs mia_ mais dont mia_state a disparu, lui,
    // vient de perdre la seule copie complète de sa fiche : mia_state est
    ...
    // allégé par mia-attr-map (ni code des mods, ni vignettes d'inventaire) :
    ...
    // danger, même gel — sans quoi vider mia_state serait PLUS destructeur que
    ...
    // de mia-attr-map.js (cache du navigateur) ne connaît pas cette constante,
    ...
    mem["mia-perso"] = JSON.stringify(state);
    mem["mia-cards"] = "{}";
    mem["mia-persos"] = "[]";     // pas de bibliothèque multi-perso dans Roll20
    ...
    // sauvegarde, qui lit mem["mia-perso"]
  ...
  window.addEventListener("message", function (ev) {
    ...
    if (d.type === "hydrate" && (!d.charId || d.charId === CHAR_ID)) {
      ...
      // dans son garde-fou — écraserait mem["mia-perso"], donc la saisie en
    ...
    else if (d.type === "take" && d.payload) window.__miaTake(d.payload);
    // liste des joueurs connectés, en réponse à __miaPlayers
  ...
  if (STANDALONE) {
    note("Cette page est le cœur de l'extension « Fiche MIA sur Roll20 » : elle " +
         "s'affiche dans l'onglet Fiche MIA d'un personnage Roll20 et n'a rien à " +
  ...
  // Réclamer les Attributes mia_* au pont d20, avec relances : le pont vient
  ...
  (function ask() {
    ...
    if (tries > 40) {             // ~20 s sans réponse : le pont n'est pas là
      note("Roll20 n'a pas répondu. Fermer et rouvrir l'onglet « Fiche MIA » ; " +
